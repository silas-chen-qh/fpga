...
parameter BAUT = 9600;
parameter  SEND_MAX = 50_000_000;
parameter  CLK_RATE = 50_000_000;
parameter BAUT_CNT_MAX = CLK_RATE/BAUT;
...
//位计数器
 always@(posedge clk or posedge rst)
    if(rst)
        bit_cnt <= 0;
    else if(baut_cnt == BAUT_CNT_MAX-1)begin
        if(bit_cnt == 9)
            bit_cnt <= 0;
        else 
            bit_cnt <= bit_cnt + 1;
...
//    if(rst)
//        led <= 1;
//    else if((bit_cnt == 9)&&(baut_cnt == BAUT_CNT_MAX - 1))
//        led <= ~led;

assign w_send_complete = ((bit_cnt == 9)&&(baut_cnt == BAUT_CNT_MAX - 1 ));

always@(posedge clk)
    send_complete <= w_send_complete;
